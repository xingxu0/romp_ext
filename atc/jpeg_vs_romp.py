...
for i in i_:
	x.append(i)
	j.append(i*290.0/1000)
	r.append(i*290.0*(1-saving)/1000)
	t.append(7.0)
print j
print r
f = plt.figure()
...
for i in range(len(i_)):
	...
	ax.text(r_x[i], 1.16*(r[i]+t[i]), str(s)+"%",  fontsize=24)
...
ax.legend(["JPEG", "ROMP-Table", "ROMP-Image"], 2, fontsize = legend_font_size)
...
plt.savefig('jpeg_vs_romp.png')
plt.savefig('jpeg_vs_romp.eps')
plt.close()
